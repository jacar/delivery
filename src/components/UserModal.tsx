...
export default function UserModal({ isOpen, onClose, user }: UserModalProps) {
  ...
  useEffect(() => {
    if (user) {
      ...
      setAprobado(user.aprobado || false);
    }
  }, [user]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await import('../services/pedidoService').then(s => s.actualizarDatosUsuario(user.uid, { 
        nombre, 
        telefono,
        aprobado 
      } as any)); 
            ...
            <form onSubmit={handleSave} className="p-8 space-y-6">
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl">
                  <div>
                    <p className="text-xs font-black text-gray-900 uppercase tracking-tight">Estado de Aprobación</p>
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                      {aprobado ? '✅ Usuario Aprobado' : '⏳ Pendiente de Aprobación'}
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={() => setAprobado(!aprobado)}
                    className={`w-12 h-6 rounded-full transition-all relative ${aprobado ? 'bg-green-500' : 'bg-gray-300'}`}
                  >
                    <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${aprobado ? 'left-7' : 'left-1'}`} />
                  </button>
                </div>
              </div>
