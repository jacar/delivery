...
import { createPedido, listenMisPedidosCliente, getUsuario } from '../services/pedidoService';
import { Usuario, Pedido, PedidoTipo } from '../types';
import { MapPin, Package, ShoppingCart, Clock, CheckCircle2, Bike, AlertCircle, Plus, History, User, Loader2, Navigation, X } from 'lucide-react';
...
export default function ClienteView({ userData, activeTab: propActiveTab }: ClienteViewProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!cleanRecogida || !cleanEntrega || !cleanDesc) {
        throw new Error('Todos los campos son obligatorios');
      }

      const mockRecogida = { lat: baseLat, lng: baseLng, direccion: cleanRecogida };
      const mockEntrega = { lat: baseLat + 0.01, lng: baseLng + 0.01, direccion: cleanEntrega };
      
      await createPedido(userData.uid, userData.nombre, userData.telefono || '', tipo, cleanDesc, mockRecogida, mockEntrega);
